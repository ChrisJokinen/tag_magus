...
This pull and push happens across all members of a team with one member acting as a project quality control and project manager. This person will then push his/her local target directory to a staging web server, that shares the same database as production. Once the changes are confirmed to work then the code is pushed live. In the event that an update is found to caused an error with another section of the production site a few options will be available: 
	...
	2) if you can quickly ID the offending markup on the site, you can change it manually. This get the site fuctional while the source that generated it can be traced down.
	



	
-- Addressing the largest issues with deperate systems.
...
For files all development system simply need to view the production server. so a full path should be used for all imagess. One area of contention will be in dealing with secured files. I may need to design a database system that makes a fake link to the real file, obfuscated. or I could make secured files a part of the database using blobs. either way recording this in the database will make managaging it easier while still providing security.
